...
/* global configurator */
/* global MockNavigatorSettings, MockNavigatormozApps */
/* global IccHelper, App, app, MocksHelper  */
...
suite('configurator.js >', function() {
  ...
  test('Sections >', function() {
    var req = requests[0];
    req.responseText = JSON.stringify(confGridOK);
    req.onload();
  ...
  function assertSV(xhrOk) {
    ...
    req = requests[1];
    req.responseText = JSON.stringify(confSV);
    var svApp;
    if (xhrOk) {
      req.onload();
      svApp =
